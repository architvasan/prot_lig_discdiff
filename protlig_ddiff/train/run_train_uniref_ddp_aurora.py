...
class OptimizedUniRef50Trainer:
    ...
    use_subs_loss: bool = True  # Use SUBS loss instead of SEDD loss
    ...
    def setup_model(self):
        ...
        self.model = DiscDiffModel(self.cfg).to(self.device)
    ...
    def compute_loss(self, batch):
        """Compute loss - SUBS or original SEDD based on configuration."""
        ...
        # Use SUBS loss if enabled
        if getattr(self, 'use_subs_loss', True):
            return self._compute_subs_loss(batch)
        else:
            return self._compute_original_loss(batch)

    def _compute_subs_loss(self, batch):
        ...
        if (hasattr(self.cfg, 'curriculum') and self.cfg.curriculum.enabled and
            ...
            from noise_lib import sample_timesteps_curriculum
            ...
            if self.state['step'] % 100 == 0:
                from noise_lib import get_curriculum_stats
        ...
        # Apply curriculum learning to corruption process
        if (hasattr(self.cfg, 'curriculum') and self.cfg.curriculum.enabled and
        ...
        try:
            ...
            print(f"❌ Error in SUBS loss computation: {e}")
            print(f"   perturbed_batch shape: {perturbed_batch.shape}")
            print(f"   sigma shape: {sigma.shape}")
            print(f"   batch shape: {batch.shape}")
            raise e

    def _compute_original_loss(self, batch):
        """Compute original SEDD loss (fallback)."""
        # Sample timesteps with optional probabilistic curriculum
        if (hasattr(self.cfg, 'curriculum') and self.cfg.curriculum.enabled and
            hasattr(self.cfg.curriculum, 'probabilistic') and self.cfg.curriculum.probabilistic):
            # Use probabilistic curriculum: bias timestep sampling toward lower noise early in training
        ...
        # Forward pass with device-aware autocast and shape validation (original SEDD)
        try:
            device_type = str(self.device).split(':')[0]
            ...
            if self.use_amp and device_type == 'xpu':
                with torch.xpu.amp.autocast():
                    ...
                    log_score = model_to_use(perturbed_batch, sigma, use_subs=False)
